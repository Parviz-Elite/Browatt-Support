...
<div class="min-h-screen bg-[#f4f8fb] text-slate-950">
        ...
        <div class="flex h-full flex-col">
            ...
            <nav class="flex flex-1 flex-col gap-7">
                <a
                    ...
                    class={`flex h-12 items-center gap-3 rounded-2xl px-4 text-sm font-black transition ${
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-1.5">
                            {#each group.items as item}
                                ...
                                <Link
                                    href={item.href}
                                    class={`flex h-11 items-center gap-3 rounded-2xl px-4 text-sm font-bold transition ${
                                        active ? 'bg-red-50 text-slate-950' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950'
                                    }`}
                                >
                                    {#if item.icon === 'shield'}
    ...
    {#if mobileMenuOpen}
        ...
        ></button>
        <aside class="fixed inset-y-0 right-0 z-50 w-[min(21rem,88vw)] bg-white px-4 py-5 shadow-2xl xl:hidden">
    ...
    <div class="min-h-screen xl:pr-72">
        ...
        <main class="min-h-[calc(100vh-5rem)] px-4 py-5 sm:px-6 xl:px-8">
            <slot />
        </main>
    </div>
</div>
